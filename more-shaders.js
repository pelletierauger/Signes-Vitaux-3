...
void main() {
    ...
    // gl_FragColor = vec4(pix, 0.25 - pix2, 0.75 - pix * 0.5, 1.0);
    // warm blue, purple and cyan
    float pix = 1.0 - c * 1.5 + (rando * 0.1);
    float pix2 = 1.0 - c * 0.45 + (rando * 0.1);
    gl_FragColor = vec4(0.125 - pix * 2., sin(c2 * 2.) * 1.25 - c2, (1. - cos(pix2 * 0.1) + pix2 * 1.5), 1.0);
    // gl_FragColor = vec4(0.475 - c2, 0.125 - pix2, pix2 - 0.5, 1.0);
    // gl_FragColor = vec4(vec3(d) + vec3(d2), 1.0);
}
`);
...
float circ(float speed, float size, float vx, float vy, float dist) {
  ...
  // float y = sin(time * speed) * dist * 0.012 - 0.25;
  float t = time;
  float x = 10. * cos(t * speed * 100.0) * dist * (sin(t)) * 0.12 - 0.425;
  float y = 10. * sin(t * speed * 100.0) * dist * (sin(t)) * 0.12 - 0.25;
  // float x = cos(time * speed) * dist * abs(sin(time * 0.01) * 1.0) - 0.425;
  // float y = sin(time * speed) * dist * abs(sin(time * 0.01) * 1.0) - 0.25;
  vec2 v = vec2(vx + x, vy + y);
  float d = 1.0 / length(v * size);
...
float circ(float speed, float size, float vx, float vy, float dist) {
  ...
  // float y = sin(time * speed) * dist * 0.012 - 0.25;
  float t = time;
  float x = 15. * cos(t * speed * 1.0) * dist * (sin(t)) * 0.12 - 0.425;
  float y = 15. * sin(t * speed * 1.0) * dist * (sin(t)) * 0.12 - 0.25;
  // float x = cos(time * speed) * dist * abs(sin(time * 0.01) * 1.0) - 0.425;
  // float y = sin(time * speed) * dist * abs(sin(time * 0.01) * 1.0) - 0.25;
  vec2 v = vec2(vx + x, vy + y);
  float d = 1.0 / length(v * size);
d = sin(d * sin(time * 10.) * 200.);
  return d;
}
float rand(vec2 co){
    return fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * 43758.5453 * (2.0 + sin(time)));
...
// -------------------------------------------//
//---------puddle-2---entrelacs-gigantesques--//
// -------------------------------------------//
setBothShaders(`
precision mediump float;
// this is the same variable we declared in the vertex shader
...
// -------------------------------------------//
//-----puddle-2---entrelacs-gigantesques-2----//
// -------------------------------------------//
setBothShaders(`
precision mediump float;
// this is the same variable we declared in the vertex shader
