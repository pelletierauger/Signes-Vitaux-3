...
float circ(float speed, float size, float vx, float vy, float dist) {
  ...
  float x = cos(time * speed) * dist * (1.0 + sin(time * 0.005) * 1.5) * 0.12 - 0.425;
  float y = sin(time * speed) * dist * (1.0 + sin(time * 0.005) * 1.5) * 0.12 - 0.25;
...
void main() {
    ...
    gl_FragColor = vec4(c2 * 1.0, 1.25 - pix2 * 1.2, (0.75 - pix * 0.25) * 0.55, 1.0);
